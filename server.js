...
function saveRules() {
    try {
        console.log(`💾 Сохраняем ${rules.length} правил в файл ${DATA_FILE}...`);
        const jsonData = JSON.stringify(rules, null, 2);
        console.log(`📝 Подготовлено данных для записи: ${jsonData.length} символов`);
        fs.writeFileSync(DATA_FILE, jsonData);
        console.log(`✅ Правила успешно сохранены в ${DATA_FILE}`);
    } catch (error) {
